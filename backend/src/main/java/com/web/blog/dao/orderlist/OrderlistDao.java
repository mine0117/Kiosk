...
public interface OrderlistDao extends JpaRepository<Orderlist, String> {
	...
	@Query(nativeQuery = true, value = "SELECT b.image, b.price, count(o.menuid) AS c FROM orderlist o left JOIN branch b ON o.menuid = b.menuid WHERE DATE_FORMAT(o.orderdate,'%h') = DATE_FORMAT(now(),'%h') GROUP BY o.menuid ORDER BY c DESC LIMIT 3")
