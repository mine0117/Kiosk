...
def train(train_dir, verbose=False):
    ...
    # X = []
    # y = []
    X = loadtxt("/var/lib/jenkins/workspace/sucheol's/face_classifier/output2.csv",
                delimiter=',').tolist()
    y = loadtxt(
        "/var/lib/jenkins/workspace/sucheol's/face_classifier/output3.csv", dtype=str).tolist()
    ...
    model_path = "C:/do/face_classifier/face_classifier/"
...
def dump_model(classifier, params=None, path=None):
    ...
    model_path = "C:/do/face_classifier/"
    if path is None:
...
def predictionss(X_img_path: str, detection_model: str, jitters: int, encoding_models: str,
    ...
    with open("/var/lib/jenkins/workspace/sucheol's/face_classifier/model.pkl", 'rb') as f:
...
if __name__ == "__main__":
    ...
    classifier = train("/var/lib/jenkins/workspace/sucheol's/face_classifier/train")

    # STEP 2: Using the trained classifier, make predictions for unknown images
    model_path = "/var/lib/jenkins/workspace/sucheol's/face_classifier/"

    classifier_folder = os.path.join(model_path)
    classifier_file = os.path.join(classifier_folder, "config")

    CONFIG_FILE = "/var/lib/jenkins/workspace/sucheol's/face_classifier/config.json"
    ...
    full_path = "/var/lib/jenkins/workspace/sucheol's/face_classifier/test/"
